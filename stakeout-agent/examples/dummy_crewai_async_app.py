"""
Dummy CrewAI application to manually verify stakeout-agent monitoring (async).

Run MongoDB first:
    docker compose up -d mongo

Then run this script:
    uv run python examples/dummy_crewai_async_app.py
...
async def main():
    ...
    monitor = AsyncCrewAIMonitorCallback(crew_id="dummy_crew_async", thread_id="thread_001")
    ...
    run = db.runs.find_one({"graph_id": "dummy_crew_async"}, sort=[("started_at", -1)])
    if run is None:
